...
SLAM (Simultaneous Localization and Mapping) integration for the LeRemix robot using [slam_toolbox](https://github.com/SteveMacenski/slam_toolbox).

This package provides custom launch files and configurations specifically tailored for the LeRemix robot's hardware setup.

## Overview

This package wraps slam_toolbox with LeRemix-specific configurations for:
...
```bash
ros2 launch leremix_slam online_async_launch.py
```
...
2. **Launch localization**:
   ```bash
   ros2 launch leremix_slam localization_launch.py
   ```
...
```bash
ros2 launch leremix_slam lifelong_launch.py
```
...
## Integration with LeRemix Bringup

To automatically start SLAM with your robot, you can include this launch file in `leremix_bringup`:
...
ros2 pkg prefix leremix_slam
# Or use it directly from source:
./ros/src/leremix_slam/scripts/slam_control.sh
